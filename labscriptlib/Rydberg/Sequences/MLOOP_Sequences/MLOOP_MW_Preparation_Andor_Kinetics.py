...
if __name__ == '__main__':
    ...
    probe_duration = 100e-6#300e-6 #30e-6 #100e-6#300e-6#300e-6 #50e-6 #150e-6
    control_duration = probe_duration #10e-6
    preparation_duration = 5e-6
    ...
    agilentE8257D.set_freq_amp(MW_PREP_FREQ,-2) # set MW freq in GHz, pow in dBm
    ##################################
    ...
    Total_points = 200
    t_second_flat = 0.0
    t_first_flat = preparation_duration*10**6 - 0.0 # preparation duration - x us of ramping up
    t_off = time_btw_prep_detect*10**6
    t_first_gauss = 0.0 # x us of ramping up
    ...
    second_pulse_fraction = MW_PREP_VOLT #0.0 #MW_PREP_VOLT #0.0 #MW_PREP_VOLT #3.0
    off_fraction = MW_PREP_VOLT #0.0 #MW_PREP_VOLT #0.0 #MW_PREP_VOLT #3.0
...
##### MEASURE PREPARATION BEAM SCATTERING RATE
    # utils.jump_from_previous_value(probe_aom_power, t, 2.0)#0.6)
    # probe_aom_switch.enable(t)
    # t+= PREP_TIME_RATE_MEASUREMENT #100e-6
    # utils.jump_from_previous_value(probe_aom_power, t, 0.0)#0.6)
...
#####
    ...
    # utils.jump_from_previous_value(control_aom_power, t, 0.2)#0.3)#0.2)#0.3)# 2.0) #2.0)
    # control_aom_switch.enable(t)   
    control_aom_switch.disable(t+control_duration)
    utils.jump_from_previous_value(control_aom_power, t+control_duration, 0.0)
    utils.jump_from_previous_value(sacher_aom_power, t, 0.6) #0.4)#0.8)#0.4)#0.5)#0.6)#0.8)# 0.6)#0.2)#1.2)# 0.4)#1)#1.2)#0.6)#0.4) # 0.07 #0.6
