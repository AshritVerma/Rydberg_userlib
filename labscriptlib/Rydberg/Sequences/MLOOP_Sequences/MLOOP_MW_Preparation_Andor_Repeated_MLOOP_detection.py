...
if __name__ == '__main__':
    ...
    probe_duration = round(eval("MLOOP_DETECTION_TIME_INTERVAL{:02d}".format(0)),6) #5e-6 #10e-6#5e-6#30e-6#300e-6 #30e-6 #100e-6#300e-6#300e-6 #50e-6 #150e-6
    ...
    pts_probe.program_single_freq(MLOOP_DETECTION_PROBE_FREQ)#(1145)
    ...
    first_pulse_fraction = 0.75# for 25mV on PD: 3.0 #1.2 #2.0
    ...
    for _ in np.arange(N_repeat):
        ...
        # num_points = MLOOP_DETECTION_NUM_POINTS

        # control_aom_switch.enable(t)
        # sacher_aom_switch.enable(t+1.5e-6)
        # print("Time before num_points {t:.9f}".format(t=t))
        # for i in range(num_points):
        #     interval_time = round(eval("MLOOP_DETECTION_TIME_INTERVAL{:02d}".format(0)),6) #eval("MLOOP_DETECTION_TIME_INTERVAL{:02d}".format(i))
        #     print("Time interval_time {t:.9f}".format(t=interval_time))
        #     # jump probe freq?
        #     utils.jump_from_previous_value(sacher_aom_power, t, eval("MLOOP_DETECTION_PROBE_POWER{:02d}".format(0)))
        #     utils.jump_from_previous_value(control_aom_power, t, eval("MLOOP_DETECTION_CONTROL_POWER{:02d}".format(0)))
        #     print("Time in num_points {t:.9f}".format(t=t))
        #     t+= interval_time
        #     print("Time after num_points {t:.9f}".format(t=t))
        # utils.jump_from_previous_value(control_aom_power, t+control_duration, 0.0)
        # control_aom_switch.disable(t + 3e-6)
        # sacher_aom_switch.disable(t + 1.5e-6)
        # MW_TTL.disable(t + 1.5e-6) # already disabled

        # # normal sequence changed
        utils.jump_from_previous_value(control_aom_power, t, eval("MLOOP_DETECTION_CONTROL_POWER{:02d}".format(0)))# for side use:0.2)#0.2)#0.3)# 2.0) #2.0)
        control_aom_switch.enable(t)   
        ...
        utils.jump_from_previous_value(sacher_aom_power, t,  eval("MLOOP_DETECTION_PROBE_POWER"))#0.45)#0.45)#0.7)#0.6)#0.4)#0.5)#0.6)#0.8)# 0.6)#0.2)#1.2)# 0.4)#1)#1.2)#0.6)#0.4) # 0.07 #0.6
        sacher_aom_switch.enable(t + 1.5e-6)
        t += round(eval("MLOOP_DETECTION_TIME_INTERVAL{:02d}".format(0)),6) #eval("MLOOP_DETECTION_TIME_INTERVAL{:02d}".format(i))#50e-6
