...
if __name__ == '__main__':
    ...
    for i in range(num_points):
        total_time += round(eval("MLOOP_DETECTION_TIME_INTERVAL{:02d}".format(i)),num_round_time) #eval("MLOOP_DETECTION_TIME_INTERVAL{:02d}".format(i))#50e-6

    probe_duration = total_time #round(eval("MLOOP_DETECTION_TIME_INTERVAL{:02d}".format(0)),6) #5e-6 #10e-6#5e-6#30e-6#300e-6 #30e-6 #100e-6#300e-6#300e-6 #50e-6 #150e-6
    ...
    for _ in np.arange(N_repeat):
        ...
        for i in range(num_points):
            utils.jump_from_previous_value(control_aom_power, t, round(eval("MLOOP_DETECTION_CONTROL_POWER{:02d}".format(i)),num_round_volt))# for side use:0.2)#0.2)#0.3)# 2.0) #2.0)
            utils.jump_from_previous_value(sacher_aom_power, t, round(eval("MLOOP_DETECTION_PROBE_POWER{:02d}".format(i)),num_round_volt))#0.45)#0.45)#0.7)#0.6)#0.4)#0.5)#0.6)#0.8)# 0.6)#0.2)#1.2)# 0.4)#1)#1.2)#0.6)#0.4) # 0.07 #0.6
            t += round(eval("MLOOP_DETECTION_TIME_INTERVAL{:02d}".format(i)),num_round_time) #eval("MLOOP_DETECTION_TIME_INTERVAL{:02d}".format(i))#50e-6
