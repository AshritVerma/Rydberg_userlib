...
if __name__ == '__main__':
    ...
    time_btw_prep_detect = 15e-6
    N_repeat = PREP_SEQ_REPEAT
    repeat_dead_time =  20e-6
    GATE_TIME = 180e-3
    ##################################
    ...
    first_pulse_fraction = 1.5 #0.75# for 25mV on PD: 3.0 #1.2 #2.0
    ...
    for i in np.arange(N_repeat):
        ...
        utils.jump_from_previous_value(control_aom_power, t, 0.15) #0.4)# for side use:0.1)#0.05)#0.1)# 0.3)#0.2)# 2.0) #2.0)
        ...
        is_it_top_control.disable(t + 0.6e-6) # use if you want top control
        ...
        is_it_top_control.enable(t + 3e-6) # use if you want top control
        t += repeat_dead_time
