...
if __name__ == '__main__':
    ...
    agilentE8257D.set_freq_amp(MW_PREP_FREQ, 4.0)#-2.0)# MW_RF_POW)#-2) # set MW freq in GHz, pow in dBm
    ##################################
    ...
    first_pulse_fraction = 1.5# 0.75 #1.5 #0.75# for 25mV on PD: 3.0 #1.2 #2.0
    ...
    # use this for prep and detect:
    second_pulse_fraction = 0.0 #0.0 #MW_PREP_VOLT #3.0
    off_fraction = 0.0 #0.0
    # use this for MW EIT:
    # second_pulse_fraction = MW_PREP_VOLT
    # off_fraction = MW_PREP_VOLT
    ...
    # freq_to_um_conversion = 0.0346 # MHz/um
...
######## Make strings for prep in one detect in other state:   
    ...
    for i in range(ntraps_x):
        if i in traps_to_keep10:
            ...
            if (i != traps_to_keep10[-1]):
                string_x10 += " %g*np.cos(2*np.pi*x*%g +%g) +" %(1/len(traps_to_keep10), round(freq_atom,4), random_phase)
    ...
    ### 1 beam string:
    # string_x1 = "lambda x: np.cos(2*np.pi*x*.32)"
    string_x1 = " lambda x: "
    traps_to_keep2 = [5] #[0,9]
    for i in range(ntraps_x):
        if i in traps_to_keep2:
            atom_pos = i - int(0.5*ntraps_x)
            freq_atom = (central_freq + atom_pos*freq_to_um_conversion*separation_um)/sampling_rate
            ...
            random_phase = random_phase_x[i]
            if (i != traps_to_keep2[-1]):
                string_x1 += " %g*np.cos(2*np.pi*x*%g +%g) +" %(2.0/len(traps_to_keep4), round(freq_atom,4), random_phase)
                # string_x += " %g*np.cos(2*np.pi*x*%g +%g) +" %(amplitudes_x[i], round(freq_atom,4), random_phase)
    ...
    # freq_atom = (central_freq)/sampling_rate
    # string_x1 += " %g*np.cos(2*np.pi*x*%g +%g) " %(1.0, round(freq_atom,4), 0.0)
    # print(string_x1)
    ##################################
    ...
    # spectrum_awg.add_segment(.001, {0: string_x, 1:string_y}, N_samples) # (2*np.pi*x*.32) is 40 MHz
    # spectrum_awg.add_segment(.001, {0: string_x4, 1:string_y}, N_samples) # (2*np.pi*x*.32) is 40 MHz
    # spectrum_awg.add_segment(.001, {0: string_x2, 1:string_x4}, N_samples) # (2*np.pi*x*.32) is 40 MHz
    #spectrum_awg.add_segment(.001, {0: string_x4, 1:string_x4}, N_samples) # (2*np.pi*x*.32) is 40 MHz
    ...
    # spectrum_awg.add_segment(t, {0: string_x, 1:string_y}, N_samples) # (2*np.pi*x*.32) is 40 MHz
    # spectrum_awg.add_segment(t, {0: string_x4, 1:string_y}, N_samples) # (2*np.pi*x*.32) is 40 MHz
    # spectrum_awg.add_segment(t, {0: string_x2, 1:string_x4}, N_samples) # (2*np.pi*x*.32) is 40 MHz
    # spectrum_awg.add_segment(t, {0: string_x4, 1:string_x4}, N_samples) # (2*np.pi*x*.32) is 40 MHz
    ...
    # Add optical pumping:
    t+=100e-6
    utils.jump_from_previous_value(op_aom_power, t, OP_OPTIMIZATION_POWER)# 1.5 )#0.5)#0.8)#0.6)
    ...
    # t+=1e-3
    # utils.jump_from_previous_value(probe_aom_power, t, 0.5)#1.2)# 0.4)#1)#1.2)#0.6)#0.4) # 0.07 #0.6
    # probe_aom_switch.enable(t)
    # t+= 50e-6 #PREP_TIME_RATE_MEASUREMENT #300e-6 #2e-3
    ...
    # switch_480AODconfig_to_detect.enable(t)
    # t+=0.0001
    ...
    for i in np.arange(N_repeat):
        ...
        utils.jump_from_previous_value(control_aom_power, t, 0.35)#CONTROL_PREP_VOLT) #0.4)# for side use:0.1)#0.05)#0.1)# 0.3)#0.2)# 2.0) #2.0)
        ...
        utils.jump_from_previous_value(control_aom_power, t, 2.0)#0.8) #2.0)# for side use:0.2)#0.2)#0.3)# 2.0) #2.0)
        control_aom_switch.enable(t) 
