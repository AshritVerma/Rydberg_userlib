...
if __name__ == '__main__':
    ...
    probe_duration = 40e-6 #10e-6 #5e-6 #15e-6#10e-6#5e-6#30e-6#300e-6 #30e-6 #100e-6#300e-6#300e-6 #50e-6 #150e-6
    ...
    hamamatsu.camera_attributes['exposure'] = (probe_duration+ time_btw_prep_detect + preparation_duration + repeat_dead_time )*N_repeat+ 50e-6#1e-3
    ...
    # Add optical pumping:
    t+=100e-6
    utils.jump_from_previous_value(op_aom_power, t, 1.0) #0.5)#0.8)#0.6)
    ...
    for i in np.arange(N_repeat):
        ...
        utils.jump_from_previous_value(sacher_aom_power, t, PROBE_RATE_SINGLESHOT)#0.45)#0.45)#0.7)#0.6)#0.4)#0.5)#0.6)#0.8)# 0.6)#0.2)#1.2)# 0.4)#1)#1.2)#0.6)#0.4) # 0.07 #0.6
