...
if __name__ == '__main__':
    ...
    # ## Add preparation light along OP to do optical pumping
    t+=1e-3
    utils.jump_from_previous_value(probe_aom_power, t, 0.25)#1.2)# 0.4)#1)#1.2)#0.6)#0.4) # 0.07 #0.6
    probe_aom_switch.enable(t)
    utils.jump_from_previous_value(repump_aom_power, t, 2.0)#2)
    repump_aom_switch.enable(t)
    t+= 2e-3#2e-3 #PREP_TIME_RATE_MEASUREMENT #300e-6 #2e-3
    utils.jump_from_previous_value(repump_aom_power, t, 0)
    repump_aom_switch.disable(t)
