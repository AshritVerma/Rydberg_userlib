...
nloc_x = int((slm.nx-get_loc_sz)/2)
nloc_y = int((slm.ny-get_loc_sz)/2)
...
get_loc_phase = np.zeros(shape=(slm.ny,slm.nx))
...
curr_path = "Z:\\2022-data\\2022-04\\20220415-wf_correction\\"
...
# # Get the location of first order from user clicks
get_loc_phase_bmp = slm.convert_bmp(get_loc_phase, bit_depth=255.*mod_depth_get_loc)
...
nmode_x = int(np.ceil(slm.nx/mode_sz))
nmode_y = int(np.ceil(slm.ny/mode_sz))

# Index of reference mode (at quarter position of SLM)
nmode_center_x = int(slm.nx/mode_sz/2)
nmode_center_y = int(slm.ny/mode_sz/2)

# Index of reference mode (at quarter position of SLM)
nmode_ref_x = int(slm.nx/mode_sz/3)
nmode_ref_y = int(slm.ny/mode_sz/3)
...
base_phase = np.zeros(shape=(slm.ny,slm.nx))
...
# set the reference mode
max_base_phase = np.zeros(shape=(slm.ny,slm.nx))
...
for currmode_y in range(nmode_y):
	for currmode_x in range(nmode_x):
		...
		ref_base_phase = np.zeros(shape=(slm.ny,slm.nx))
